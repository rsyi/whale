...
class BigQueryWatermarkExtractor(BaseBigQueryExtractor):
    ...
    def _get_partitions(self, table, tableRef):
        ...
        result = (
            self.bigquery_service.jobs()
            .query(projectId=self.project_id, body=body)
            .execute()
        )
        ...
        return [
            PartitionInfo(row["f"][0]["v"], row["f"][1]["v"]) for row in result["rows"]
        ]
    ...
    def _get_partition_watermarks(self, table, tableRef, partitions):
        ...
        low_wm = Watermark(
            datetime.datetime.fromtimestamp(float(low.epoch_created)).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
            "bigquery",
            tableRef["datasetId"],
        ...
        high_wm = Watermark(
            datetime.datetime.fromtimestamp(float(high.epoch_created)).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
            "bigquery",
            tableRef["datasetId"],
