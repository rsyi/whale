    ...
    def init(self, conf: ConfigTree) -> None:
        ...
        if self.key_path:
            ...
            if self.cred_key:
                ...
                credentials, _ = \
                    google.auth.default(scopes=self._DEFAULT_SCOPES)

        http = httplib2.Http()
        authed_http = \
            google_auth_httplib2.AuthorizedHttp(credentials, http=http)
        self.bigquery_service = \
            build('bigquery', 'v2', http=authed_http, cache_discovery=False)
        self.logging_service = \
            build('logging', 'v2', http=authed_http, cache_discovery=False)
